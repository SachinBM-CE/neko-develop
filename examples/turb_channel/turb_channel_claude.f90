...
module user
  ...
  use rlwm, only: rlwm_t  ! Changed from RLWM to rlwm
  use math
...
contains
  ...
  subroutine user_setup(user)
    ...
    user%initial_conditions => initial_conditions
    user%mesh_setup => user_mesh_scale
    user%compute => usercheck  ! Add TorchFort callback
  end subroutine user_setup
  ...
  subroutine usercheck(time)
    ...
    select type (bc)
       ...
       select type(this => wall_bc%wall_model)
          ...
          type is (rlwm_t)  ! Changed from RLWM_t to rlwm_t
            
          if (allocated(this%state)) then
        
             ! Gather current state from all ranks
             call mpi_gatherv(this%state, 3*this%n_nodes, MPI_DOUBLE_PRECISION, &
                           this%global_state, 3*this%recvcounts, 3*this%displs, MPI_DOUBLE_PRECISION, &
                           0, NEKO_COMM, ierr)
             if (pe_rank == 0) then
             ...
             call mpi_gatherv(this%reward, this%n_nodes, MPI_DOUBLE_PRECISION, &
             ...
             call mpi_gatherv(this%terminal, this%n_nodes, MPI_DOUBLE_PRECISION, &
          ...
          if ((mod(tstep, this%tsteps_rl) == 0) .or. (abs(t - time%end_time) .le. 1.0e-3_rp)) then
             reward_sum = glsum(this%total_reward(:), this%n_nodes)
          ...
          do i = 1, this%n_nodes
             ...
             if ((mod(tstep, this%tsteps_rl) == 0) .or. (abs(t - time%end_time) .le. 1.0e-3_rp)) then
                this%terminal(i) = 1.0_rp
                this%total_reward(i) = 0.0_rp
             else
                this%terminal(i) = 0.0_rp
             end if

          end do
          
          call copy(this%terminal_older, this%terminal_old, size(this%terminal_old))
          call copy(this%terminal_old, this%terminal, size(this%terminal))
